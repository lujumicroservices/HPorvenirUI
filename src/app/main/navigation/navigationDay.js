...
function DialogDayViewer(props, ref){
    ...
    function loadDayInfo(year,month,day){
        navigationService.loadDay(year,month,day).then((days) => {
            days.map	    
            days.thumb = days.thumb.map(function(day) {
                return `${days.urlPrefix}/${days.container}/${day}${days.shareKey}`;
            });
            days.pages = days.pages.map(function(day) {
                return `${days.urlPrefix}/${days.container}/${day}`;
            });
    ...
    open={openDialog} onClose={handleCloseDialog} aria-labelledby="form-dialog-title">
        <AppBar position="static" elevation={0}>
            <Toolbar className="flex w-full">
                <IconButton size="medium" edge="start" color="inherit" onClick={handleCloseDialog} aria-label="close">
                    <CloseIcon   />
                </IconButton>
...
{dayInfo &&
                ...
                { `${navigationService.getDayString(selectedDate.getDay())}, ${selectedDate.getDate()} De ${navigationService.getMonthString(selectedDate.getMonth())} De ${selectedDate.getFullYear()}`} 
                </Typography>
    ...
    <Paper className="w-full rounded-20 shadow flex flex-col justify-between">
      ...
      <div id="pdfcont" onWheel={handleWheel}  > 
          ...
          <Document  className={classes.document}
                ...
                file={dayInfo.pages[selectedPage]}
...
<div className="p-24">
    ...
    {dayInfo && dayInfo.thumb.map((page,i) => (	
						
        <ListItem>
            <Button className={classes.thumb} autoFocus color="inherit" onClick={() => onSelectPage(i)}>
                <img  src={page} alt=""/>
                { i==selectedPage &&
                    <Button 
